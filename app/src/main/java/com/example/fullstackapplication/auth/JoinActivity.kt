...
//            Toast.makeText(this@JoinActivity,"$email,$pw",Toast.LENGTH_SHORT).show()
            ...
            if(checkPw.isEmpty()){
                isJoin=false
                Toast.makeText(this,"비밀번호 재입력을 해주세요",Toast.LENGTH_SHORT).show()
            }
            // 비밀번호랑 재입력한 비밀번호가 똑같은지
            if(pw!=checkPw){
                isJoin=false
                Toast.makeText(this,"비밀번호를 똑같이 해주세요",Toast.LENGTH_SHORT).show()
            }
            // 비밀번호가 8자리 이상인지

            if(pw.length<8){
                isJoin=false
                Toast.makeText(this,"비밀번호를 8자리이상으로 해주세요",Toast.LENGTH_SHORT).show()
            }
            //Firebase에 규칙은 정해져있지만
            // 그 규칙을 사용자는 모르니 토스트를 통해서 알려줘야한다.
